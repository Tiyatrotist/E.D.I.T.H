...
# Offline mode badge — config'den yüklenir
def _get_model_badge():
    try:
        cfg = load_app_config()
        ...
        return f"{mode_label} · {model} · STT:{stt_model}"
    except:
        return "VOICE CORE · Windows"
...
class EdithUI:
    def __init__(self):
        ...
        cfg_boot = load_app_config()
        self._compact_mode = bool(cfg_boot.get("ui_start_compact", True))
        self._compact_prev_geometry = _geo
        ...
        self._edith_state   = "INITIALISING"
        ...
        # Compact overlay tema
        self._compact_is_dark = False
        self._compact_theme_job = None
        self._compact_void_color = None
        ...
        self._wave_edith = [random.randint(4, 26) for _ in range(18)]
        ...
        cfg = load_app_config()
        self._typewriter_enabled = bool(cfg.get("ui_typewriter_enabled", False))
        try:
            self._typewriter_delay_ms = max(0, int(cfg.get("ui_typewriter_delay_ms", 2)))
        except Exception:
            self._typewriter_delay_ms = 2
        ...
        self._build_lang_toggle(self._settings_body)
    ...
    def _toggle_compact_mode(self):
        self._compact_mode = not bool(getattr(self, "_compact_mode", False))
        if self._compact_mode:
    ...
    def _enter_compact_mode(self):
        # save state
        try:
            self._compact_prev_geometry = self.root.geometry()
        except Exception:
            pass
        self._compact_prev_fullscreen = bool(getattr(self, "_fullscreen", False))
        try:
            self._compact_prev_overrideredirect = bool(self.root.overrideredirect())
        except Exception:
            self._compact_prev_overrideredirect = False

        cfg = load_app_config()
        w = int(cfg.get("ui_compact_width", 200) or 200)
        h = int(cfg.get("ui_compact_height", 200) or 200)
        m = int(cfg.get("ui_compact_margin", 14) or 14)
        d = min(w, h)
        d = max(140, min(d, self.root.winfo_screenwidth(), self.root.winfo_screenheight()))
        ...
        self._overlay_ring_only = True
        self._compact_is_dark = False
        self.root.geometry(f"{d}x{d}+{x}+{y}")
        self._set_ring_metrics(d)
        self.bg.configure(width=d, height=d)
        self.bg.place(x=0, y=0)
        self._place_layout_widgets()
        # Uygulama tespiti başlat
        if self._compact_theme_job is None:
            self._compact_theme_job = self.root.after(800, self._check_compact_theme)

    def _exit_compact_mode(self):
        # Tema kontrolcüsünü durdur
        if self._compact_theme_job is not None:
            try:
                self.root.after_cancel(self._compact_theme_job)
            except Exception:
                pass
            self._compact_theme_job = None
        self._compact_is_dark = False
        ...
        self._overlay_ring_only = False
        if bool(getattr(self, "_compact_prev_fullscreen", False)):
            self._fullscreen = True
            self._enter_fullscreen()
            return
        self._fullscreen = False
        try:
            self.root.geometry(getattr(self, "_compact_prev_geometry", self._window_geometry))
        except Exception:
            self.root.geometry(self._window_geometry)
        self._resize_surface(*self._normal_size)

    def _check_compact_theme(self):
        """Ön planda başka uygulama varsa koyu, yoksa açık tema."""
        if not getattr(self, "_overlay_ring_only", False):
            self._compact_theme_job = None
            return
        try:
            import ctypes
            ...
            is_other = (
                fg_hwnd != 0
                and fg_hwnd != my_hwnd
                and fg_hwnd != desktop_hwnd
                and fg_hwnd != shell_hwnd
            )
            self._compact_is_dark = bool(is_other)
        except Exception:
            self._compact_is_dark = False
        self._compact_theme_job = self.root.after(700, self._check_compact_theme)

    def _set_ring_metrics(self, d: int):
        """
        ...
        self.FCX = self.W // 2
        self.FCY = self.H // 2
        # Dairesel halkaların pencereden taşmaması için dış yarıçap ölçeği.
        self.FACE = max(60, int(self.W * 0.86))
    ...
    def _layout_settings_controls(self):
        ...
        self._lang_label.place(x=0, y=218)
        self._lang_canvas.place(x=88, y=214)

    def _build_memory_controls(self):
        self._memory_body = tk.Frame(self._settings_panel, bg="#041111")
        inner_w = self._settings_geometry["panel_w"] - 24
        
        list_frame = tk.Frame(self._memory_body, bg="#041111")
        list_frame.place(x=0, y=0, width=inner_w, height=130)
        
        self._mem_listbox = tk.Listbox(
            list_frame,
            fg=C_TEXT,
            bg="#020a0a",
        ...
        self._mem_listbox.place(x=0, y=0, width=inner_w - 18, height=130)
        self._mem_listbox.bind("<<ListboxSelect>>", self._on_memory_select)
        
        scrollbar = tk.Scrollbar(list_frame, orient="vertical", command=self._mem_listbox.yview)
        scrollbar.place(x=inner_w - 16, y=0, width=16, height=130)
        self._mem_listbox.config(yscrollcommand=scrollbar.set)
        
        lbl_y = 138
        entry_h = 24
        
        tk.Label(self._memory_body, text="KATEGORİ (category):", fg=C_MID, bg="#041111", font=font_body_bold(8), anchor="w").place(x=0, y=lbl_y, width=inner_w)
        self._mem_cat_entry = tk.Entry(self._memory_body, fg=C_TEXT, bg="#020a0a", insertbackground=C_TEXT, borderwidth=0, highlightthickness=1, highlightbackground=C_DIM, highlightcolor=C_PRI, font=font_body(10))
        self._mem_cat_entry.place(x=0, y=lbl_y + 16, width=inner_w, height=entry_h)
        
        tk.Label(self._memory_body, text="ANAHTAR (key):", fg=C_MID, bg="#041111", font=font_body_bold(8), anchor="w").place(x=0, y=lbl_y + 44, width=inner_w)
        self._mem_key_entry = tk.Entry(self._memory_body, fg=C_TEXT, bg="#020a0a", insertbackground=C_TEXT, borderwidth=0, highlightthickness=1, highlightbackground=C_DIM, highlightcolor=C_PRI, font=font_body(10))
        self._mem_key_entry.place(x=0, y=lbl_y + 60, width=inner_w, height=entry_h)
        
        tk.Label(self._memory_body, text="DEĞER (value):", fg=C_MID, bg="#041111", font=font_body_bold(8), anchor="w").place(x=0, y=lbl_y + 88, width=inner_w)
        self._mem_val_entry = tk.Entry(self._memory_body, fg=C_TEXT, bg="#020a0a", insertbackground=C_TEXT, borderwidth=0, highlightthickness=1, highlightbackground=C_DIM, highlightcolor=C_PRI, font=font_body(10))
        self._mem_val_entry.place(x=0, y=lbl_y + 104, width=inner_w, height=entry_h)
        
        btn_y = lbl_y + 134
        
        self._mem_save_btn = tk.Button(
            self._memory_body,
            text="KAYDET",
            command=self._save_ui_memory,
            fg=C_BG, bg=C_PRI,
            activeforeground=C_BG, activebackground=C_BLUE,
            font=font_body_bold(9),
            borderwidth=0, cursor="hand2",
        )
        self._mem_save_btn.place(x=0, y=btn_y, width=110, height=28)
        
        self._mem_del_btn = tk.Button(
            self._memory_body,
            text="SİL",
            command=self._delete_ui_memory,
            fg=C_TEXT, bg=C_DIM,
            activeforeground=C_TEXT, activebackground=C_RED,
            font=font_body_bold(9),
            borderwidth=0, cursor="hand2",
        )
        self._mem_del_btn.place(x=120, y=btn_y, width=110, height=28)
        
        self._mem_ref_btn = tk.Button(
            self._memory_body,
            text="YENİLE",
            command=self._refresh_memory_listbox,
            fg=C_TEXT, bg=C_DIMMER,
            activeforeground=C_PRI, activebackground=C_DIM,
            font=font_body_bold(9),
            borderwidth=0, cursor="hand2",
        )
        self._mem_ref_btn.place(x=240, y=btn_y, width=inner_w - 240, height=28)
        
        self._mem_items_cache = []
        self._refresh_memory_listbox()

    def _refresh_memory_listbox(self):
        if not hasattr(self, "_mem_listbox"):
            return
        self._mem_listbox.delete(0, tk.END)
        ...
        for cat, items in mem.items():
            if isinstance(items, dict):
                for key, val in items.items():
                    ...
                    display = f"[{cat}] {key}: {value}"
                    self._mem_listbox.insert(tk.END, display)
                    self._mem_items_cache.append((cat, key, value))
            else:
                display = f"[{cat}]: {items}"
                self._mem_listbox.insert(tk.END, display)
                self._mem_items_cache.append((cat, None, items))

    def _on_memory_select(self, event):
        selection = self._mem_listbox.curselection()
        if selection:
            idx = selection[0]
            if idx < len(self._mem_items_cache):
                cat, key, val = self._mem_items_cache[idx]
                self._mem_cat_entry.delete(0, tk.END)
                self._mem_cat_entry.insert(0, cat)
                self._mem_key_entry.delete(0, tk.END)
                self._mem_key_entry.insert(0, key or "")
                self._mem_val_entry.delete(0, tk.END)
                self._mem_val_entry.insert(0, str(val))

    def _save_ui_memory(self):
        cat = self._mem_cat_entry.get().strip()
        ...
        self._mem_key_entry.delete(0, tk.END)
        self._mem_val_entry.delete(0, tk.END)
        self.write_log(f"SYS: Hafıza güncellendi ({cat}/{key})")

    def _delete_ui_memory(self):
        cat = self._mem_cat_entry.get().strip()
        key = self._mem_key_entry.get().strip()
        if not cat:
            return
        from memory.memory_manager import delete_memory
        delete_memory(category=cat, key=key)
        self._refresh_memory_listbox()
        self._mem_key_entry.delete(0, tk.END)
        self._mem_val_entry.delete(0, tk.END)
    ...
    def _refresh_settings_status(self):
        if not hasattr(self, "_settings_status_primary"):
            return
        cfg = load_app_config()
        ollama_url = str(cfg.get("ollama_api_url", "http://localhost:11434") or "").strip()
        ollama_model = str(cfg.get("ollama_model", "llama3.1") or "").strip()
        ollama_vision_model = str(cfg.get("ollama_vision_model", "") or "").strip()
        yt_key_ready = bool(str(cfg.get("youtube_api_key", "") or "").strip())
        yt_handle = str(cfg.get("youtube_channel_handle", "") or "").strip()

        vision_state = ollama_vision_model if ollama_vision_model else "otomatik vision"
        primary = [
            f"Ollama: {ollama_model}" if ollama_model else "Ollama model eksik",
            f"Vision: {vision_state}",
        ]
        if yt_key_ready and yt_handle:
            secondary = f"API: {ollama_url} · YouTube: {yt_handle}"
        else:
            secondary = f"API: {ollama_url} · YouTube ayari eksik"
    ...
    def _sync_sound_state(self):
        ...
        if enabled and self._edith_state == "THINKING":
    ...
    # ── Language Toggle ──────────────────────────────────────────────────────
    def _build_lang_toggle(self, parent=None):
        parent = parent or self.root
        self._lang_label = tk.Label(parent, text="LANGUAGE", fg=C_MID, bg=parent.cget("bg"),
                                    font=font_body_bold(8))
        self._lang_canvas = tk.Canvas(parent, width=98, height=30,
                                      bg=parent.cget("bg"), highlightthickness=0, cursor="hand2")
        self._lang_canvas.bind("<Button-1>", lambda e: self._toggle_language())
        self._draw_lang_toggle()

    def _draw_lang_toggle(self):
        c = self._lang_canvas
        BW = int(c["width"])
        BH = int(c["height"])
        c.delete("all")
        ...
        cfg = load_app_config()
        lang = str(cfg.get("language", "tr")).lower()
        is_tr = (lang == "tr")
        
        col = C_PRI if is_tr else C_BLUE
        text = "TR" if is_tr else "ENG"
    ...
    def _toggle_language(self):
        cfg = load_app_config()
        current = str(cfg.get("language", "tr")).lower()
        new_lang = "eng" if current == "tr" else "tr"
        save_app_config({"language": new_lang})
        self._draw_lang_toggle()
        
        if self.on_language_change:
            threading.Thread(target=self.on_language_change, args=(new_lang,), daemon=True).start()
    ...
    def _toggle_pause(self):
        ...
        if self.paused:
            ...
            self.write_log("SYS: EDITH duraklatıldı.")
        else:
            self.set_state("THINKING")
            self.write_log("SYS: EDITH devam ediyor...")
        self._sync_sound_state()
    ...
    def _place_layout_widgets(self):
        if getattr(self, "_compact_mode", False):
            if getattr(self, "_overlay_ring_only", False):
                # Ring-only overlay: sadece sağ üstte şeffaf halka animasyonu.
                try:
                ...
                for w in (
                    getattr(self, "_mute_canvas", None),
                    getattr(self, "_pause_canvas", None),
                    getattr(self, "_shutdown_canvas", None),
                    getattr(self, "_settings_btn_canvas", None),
            ...
            pad = 10
            self.log_frame.place(x=pad, y=pad, width=self.W - pad * 2, height=self.H - pad * 3 - INPUT_H)
            # Hide non-essential controls in compact
            for w in (getattr(self, "_mute_canvas", None),
                      getattr(self, "_pause_canvas", None),
                      getattr(self, "_shutdown_canvas", None),
                      getattr(self, "_settings_btn_canvas", None)):
                try:
    ...
    def _on_input_submit(self, event=None):
        ...
        if not text:
            return
        if self.paused:
            self.write_log("SYS: EDITH duraklatılmış durumda. Devam etmek için pause'u kapat.")
    ...
    def _state_color(self, state: str | None = None) -> str:
        effective = state or self._edith_state
        if effective == "PAUSED":
            return C_MID
    ...
    def _start_typing(self):
        ...
        if not self._typewriter_enabled:
            ...
            self.log_text.configure(state="disabled")
            self.root.after(1, self._start_typing)
            return
    ...
    def _type_char(self, text, i, tag):
        if i < len(text):
            ...
            self.log_text.see(tk.END)
            self.root.after(self._typewriter_delay_ms, self._type_char, text, i+1, tag)
        else:
    ...
    def _animate(self):
        ...
        if self.speaking and t % 3 == 0:
            self._wave_edith = [random.randint(6, 30) for _ in range(18)]
        ...
        if now - self.last_t > (0.12 if self.speaking else 0.50):
            ...
            elif self._edith_state in ("THINKING", "INITIALISING"):
    ...
    def _orb_rgb(self):
        state = "PAUSED" if self.paused else self._edith_state
    ...
    def _draw_right_panel(self, c):
        ...
        if self.paused:
            ...
            sc, st = self._state_color(self._edith_state), self._edith_state
    ...
    def _draw_orb(self, c):
        ...
        # Compact modda masaüstündeyken (is_dark=False) ince daireler buz mavisi
        if getattr(self, "_overlay_ring_only", False):
            is_dark = getattr(self, "_compact_is_dark", False)
            if not is_dark:
                R, G, B = 220, 242, 255
        ha   = self.halo_a
        field_r = int(FW * 0.49)
    ...
    def _draw(self):
        ...
        # Ring-only overlay: şeffaf arka plan, sadece orb.
        if getattr(self, "_overlay_ring_only", False):
            is_dark = getattr(self, "_compact_is_dark", False)
            void_col = "#111111" if is_dark else "#eef8fc"
            self._compact_void_color = void_col
            self._draw_orb(c)
            self._compact_void_color = None
            return

        # ── Arka plan (Sadeleştirilmiş Tam Ekran) ─────────────────────────────
        ...
        # ── Bölücü çizgiler (Kaldırıldı) ────────────────────────────────────
        
        # ── Yan paneller ──────────────────────────────────────────────────────
        self._draw_left_panel(c)
        self._draw_right_panel(c)
        # Fill central area between left and right panels
        middle_x0 = self.LEFT_W
        middle_x1 = W - self.RIGHT_W
        c.create_rectangle(middle_x0, HDR_H, middle_x1, H - FOOTER_H, fill=C_BG, outline="")
        # ── Orb ──────────────────────────────────────────────────────────────
        self._draw_orb(c)

        state_label = "PAUSED" if self.paused else self._edith_state
        state_col = self._state_color(state_label)
        c.create_text(self.FCX, self.CTRL_Y - 24, text=SYSTEM_NAME,
                      fill=C_TEXT, font=font_display(20))
        c.create_text(self.FCX, self.CTRL_Y, text=f"● {state_label.title()}",
                      fill=state_col, font=font_body_bold(12))

        # ── HEADER (Sadeleştirildi) ──────────────────────────────────────────
        c.create_rectangle(0, 0, W, HDR_H, fill="#02080a", outline="")
        c.create_line(0, HDR_H, W, HDR_H, fill="#061818", width=1)
        ...
        indicator_state = "PAUSED" if self.paused else self._edith_state
        ...
        c.create_text(W-22, 30, text=f"{sym}  {indicator_text}",
    ...
    def _show_setup_ui(self, edit_mode: bool = False):
        ...
        title = "◈ MODEL AYARLARI" if edit_mode else "◈ İLK KURULUM GEREKLİ"
        subtitle = (
            "Ollama model ayarlarinizi guncelleyin. YouTube alanlari opsiyoneldir."
            if edit_mode else
            "Ollama çalışması için model bilgilerini girin. YouTube alanlari opsiyoneldir."
        )
        ...
        tk.Label(self.setup_frame, text="OLLAMA API URL",
        ...
        current_key = str(config.get("ollama_api_url", "http://localhost:11434") or "")
        if current_key:
            self.api_entry.insert(0, current_key)

        tk.Label(self.setup_frame, text="OLLAMA MODEL",
        ...
        current_youtube_key = str(config.get("ollama_model", "llama3.1") or "")
        if current_youtube_key:
            self.youtube_api_entry.insert(0, current_youtube_key)

        tk.Label(self.setup_frame, text="OLLAMA VISION MODEL",
                 fg=C_DIM, bg="#00080d", font=font_body(12)).pack(pady=(10, 4))
        ...
        current_handle = str(config.get("ollama_vision_model", "") or "")
        if current_handle:
    ...
    def _save_api_key(self):
        ...
        save_app_config(
            {
                "ollama_api_url": key,
                "ollama_model": youtube_key,
                "ollama_vision_model": youtube_handle,
        ...
        if was_ready:
            ...
            self.set_state("LISTENING")
            self.write_log("SYS: EDITH hazır. Dinliyorum...")
